...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		...
		// movimento especial castling lado curto rook
		...
		// movimento especial castling lado grande rook
				if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
	...
	private void undoMove(Position source, Position target, Piece capturedPiece ) {
		...
		// voltando a peça que foi capturada pra sua posição de destino
		if (capturedPiece != null) {
	...
	//devolve o devido oponente do que está jogando 
	private Color opponent(Color color) {
		// se essa core for igual a branco então (?) vou retornar preto caso o contrario (:) eu retorno branco
	...
	//fazer uma logica que tem que passar por todas as peças adiversarias e verificar se elas não podem cair na casa do rei 
	...
	//testando se o rei está em checkMate
	public boolean testCheckMate(Color color) {
		if (!testCheck(color)) {
			return false;
		}
		//essa lista vai conter todas as peças da cor do metodo 
		List<Piece> list = piecesOnTheBoard.stream().filter(x ->((ChessPiece)x).getColor() == color).collect(Collectors.toList());
		// for para pecorrer todas as peças da lista 
		for(Piece p : list) {
			//1 coisa a ser feita é pegar os possiveis movimentos da pessa "p"
			boolean[][] mat = p.possibleMoves();
			for(int i=0; i<board.getRows(); i++ ) {
				for(int j=0; j<board.getColumns(); j++) {
					//testando se essa posição na matriz é um movimento possivel
					if (mat[i][j]) {
